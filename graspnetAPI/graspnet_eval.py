    ...
    **Input:**
    ...
    def eval_scene(self, scene_id, dump_folder, TOP_K = 50, return_list = False,vis = False, max_width = 0.1, last_ann_id = 256):
        ...
        **Output:**
        ...
        score_list_list = []
        collision_list_list = []
        npy_files = sorted(glob.glob(os.path.join(dump_folder, get_scene_name(scene_id), self.camera, "*.npy")))
        ann_ids = [int(os.path.splitext(os.path.basename(f))[0]) for f in npy_files]
