...
from keypoint_utils import generatePointOnLine, angle_dif, calculate_angle, calculateDegree, \
...
class KeypointGeneration:
    def __init__(self, debugMode=False, considerGripperValidity=False, pair_check_extension=0.001, pair_itrs=5,
                 angle_with_center=75, middle_points_dist=30, polygon_creation_epsilon=0.008):
        self.considerGripperValidity = considerGripperValidity
        self.pair_check_extension = pair_check_extension
        self.debugMode = debugMode
        self.angleWithCenter = angle_with_center
        self.pair_itrs = pair_itrs
        self.orthogonalAngleThresh = 15
        self.parallelAngleThresh = 20
        self.middle_points_dist = middle_points_dist
        self.polygon_creation_epsilon = polygon_creation_epsilon

    @classmethod
    def for_2d(cls, **overrides):
        """Pixel-coordinate settings taken from the original 2D generator.

        The regular constructor retains the metric/3D defaults used by existing
        point-cloud callers. The algorithm and libraries are shared.
        """
        settings = dict(
            pair_check_extension=2.0,
            pair_itrs=5,
            angle_with_center=20,
            middle_points_dist=40,
            polygon_creation_epsilon=0.005,
        )
    ...
    def find_closest_point(self, inside_points, grasp_point1, normals, radius=0.02):
        # Preserve actual surface coordinates and the corresponding normal.
        # Keep radius in the signature for compatibility with existing callers.
        closest_index = np.argmin(np.linalg.norm(
            inside_points[:, :3] - np.asarray(grasp_point1)[:3], axis=1,
        ))
        return np.copy(inside_points[closest_index]), np.copy(normals[closest_index])
    ...
    def keypoint_scored(self, contour_points, centers, radiuses, cropped_pcd, heights, contourGroups):
    ...
    def findGraspCircles(self, polygon, skeleton, allContourPoints):
        ...
        for h in skeleton.halfedges:
            if h.is_bisector:
                ...
                y1 = float(p1.y())
                y2 = float(p2.y())
                if (distance((x1, y1), (x2, y2)) >= self.middle_points_dist and float(p1.x()), float(p1.y())) in vertices and (
                        float(p2.x()), float(p2.y())) in vertices:
                    num_points = int(distance((x1, y1), (x2, y2)) // (self.middle_points_dist / 2)) + 1
    ...
    def createContours(self, path):
        ...
        elif isinstance(path, np.ndarray) and path.ndim == 2:
            if path.dtype == np.bool_ or (
                path.size and np.min(path) >= 0 and np.max(path) <= 1
            ):
                img1_gray = (path * 255).astype(np.uint8)
            else:
                img1_gray = path.astype(np.uint8)
        else:
            raise ValueError("Mask must be an image path or a 2D NumPy array")
        ...
        for contour in contours:
            if np.shape(contour)[0] > 20:
                ...
                epsilon = self.polygon_creation_epsilon * cv2.arcLength(contour, True)
    ...
    def createPolygons(self, path):
        ...
        if len(contours) == 0:
            raise ValueError("No valid contours found in the input mask")
    ...
    def returnGraspPoses(self, path, generate_gradient=True):
        ...
        try:
            ...
            raise RuntimeError("Grasp generation failed after skeleton creation") from exc
    ...
    def split_point_cloud_by_height_ovelap(self, points, normals, step_height=0.015, overlap=0.5):
        ...
        stride = step_height * (1 - overlap)
        current_height = min_height

        while current_height <= max_height:
            upper_height = current_height + step_height
            mask = (heights >= current_height) & (heights <= upper_height)
            section_points = points[mask]
            section_normals = normals[mask]

            if len(section_points) > 0:
                selected_points.append(section_points)
                selected_normals.append(section_normals)
                cross_heights.append(
                    (section_points[:, 2].max() + section_points[:, 2].min()) / 2
                )

            current_height += stride  # move up but keep overlap

        return selected_points, selected_normals, cross_heights

    def plot_polygon_skeleton_centers(self,polygon_points, skeleton, finalCenters=None, title="Skeleton and Grasp Centers"):
    
        fig, ax = plt.subplots(figsize=(6, 6))
        
        # --- Polygon boundary ---
        poly_pts = np.array(polygon_points)
        ax.plot(poly_pts[:, 0], poly_pts[:, 1], 'k-', linewidth=1.5, label='Polygon boundary')
        ...
        for h in skeleton.halfedges:
            if h.is_bisector:
                ...
                y1 = float(p1.y())
                y2 = float(p2.y())
                plt.plot([p1.x(), p2.x()], [p1.y(), p2.y()], "#DC143C", lw=2,zorder=0)

       

        # --- Grasp centers (connected line) ---
        if len(finalCenters) > 0:
            ax.scatter(finalCenters[0, :], finalCenters[1, :], linewidth=1.8, label='Grasp centers path')
    ...
    def generate_scored_grasp_poses(self, heights, points, cropped_pcd):
        if points.shape[0] == 0:
            return [], [], None
        ...
        poly_vis = shapely.geometry.Polygon(np.array(polygon_points)[::-1])
        ...
        try:
            ...
            return [], [], None
        # self.plot_polygon_skeleton_centers(polygon_points,straightSkeleton, finalCenters)
        results, scores = self.keypoint_scored(contour_points, finalCenters, radiuses, cropped_pcd, heights,
                                               contourGroups)
        if self.debugMode:
            plt.show()
        return results, scores, poly_vis
